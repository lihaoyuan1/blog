<template>
  ...
  >
    <div
      ...
      class="counter bounceInUp animated"
    ...
    <div
      ...
      :style="{
        boxShadow: isHover && isHaveBorder ? `${iconColor} 0 0 10px` : null,
...
export default {
  ...
  computed: {
    ...
    iconColor() {
      return this.isActive || this.clicking
        ? this.colorActive
        : this.colorNormal
    },
    isReachMaxClick() {
  ...
  created() {
    ...
    const iconsList = ['good', 'love', 'star']
    this.icon = this.icon.toLowerCase()
    if (!iconsList.includes(this.icon)) {
